...
eq(bare.lab.cg, '0.65', 'bare cg default');
eq(bare.lab.lym, '1.00', 'bare lym default');
eq(bare.lab.r, '0.12', 'bare residual default');

// Explicit zeros are not treated as missing.
const zeros = core.parseScenario(new URLSearchParams('v=5&cg=0&lym=0&r=0'));
eq(zeros.lab.cg, '0.00'); eq(zeros.lab.lym, '0.00'); eq(zeros.lab.r, '0.00');

// Exact fixed-point boundary behavior.
let lab = core.evaluateLab({cg:'0.10',lym:'0.20',r:'0.02'}, {});
eq(core.formatFixed(lab.derived.candidate), '0.0000', '0.10*0.20-0.02 must be exact zero');
eq(lab.derived.sign, 'zero');
eq(lab.nodes[5], false, 'exact zero cannot license positive toy descendant');

lab = core.evaluateLab({cg:'0.01',lym:'0.01',r:'0.00'}, {});
eq(core.formatFixed(lab.derived.candidate), '0.0001');
eq(lab.derived.sign, 'positive');

lab = core.evaluateLab({cg:'0.10',lym:'0.20',r:'0.03'}, {});
eq(core.formatFixed(lab.derived.candidate), '-0.0100');
eq(lab.derived.sign, 'negative');

// Withholding premises blocks downstream logic without changing exact scalar arithmetic.
lab = core.evaluateLab({cg:'0.65',lym:'1.00',r:'0.12'}, {aReflect:false});
eq(core.formatFixed(lab.derived.candidate), '0.5300');
eq(lab.nodes[3], false); eq(lab.nodes[4], false); eq(lab.nodes[5], false);

lab = core.evaluateLab({cg:'0.65',lym:'1.00',r:'0.12'}, {aOS:false});
...
let rg = core.evaluateRG({z:'0.85',ec:'0.80',epsr:'0.12'});
eq(core.formatFixed(rg.derived.gross), '0.6800'); eq(core.formatFixed(rg.derived.net), '0.5600'); eq(rg.derived.sign, 'positive');
rg = core.evaluateRG({z:'0.10',ec:'0.20',epsr:'0.02'});
eq(core.formatFixed(rg.derived.net), '0.0000'); eq(rg.derived.sign, 'zero');
rg = core.evaluateRG({z:'0.10',ec:'0.20',epsr:'0.03'});
eq(core.formatFixed(rg.derived.net), '-0.0100'); eq(rg.derived.sign, 'negative');
...
const params = core.serializeScenario(new URLSearchParams('utm_source=test'), {
  ...
  spectrum:{epsilon:'0.25',delta:'0.42'},
  rg:{z:'0.85',ec:'0.80',epsr:'0.12'}
});
eq(params.get('v'),'6'); eq(params.get('view'),'rg'); eq(params.get('lang'),'en'); eq(params.get('node'),'n4');
eq(params.get('eps'),'0.25'); eq(params.get('delta'),'0.42'); eq(params.get('z'),'0.85'); eq(params.get('ec'),'0.80'); eq(params.get('epsr'),'0.12');
...
eq(restored.rg.z,'0.85'); eq(restored.spectrum.delta,'0.42');
